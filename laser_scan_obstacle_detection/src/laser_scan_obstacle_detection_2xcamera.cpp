...
// --- 新增與修改的參數 --->
static double front_stop_distance = 0.8;
static double back_stop_distance  = 0.8;
static bool check_front = true;
static bool check_back  = true;
// ------------------------
...
int main(int argc, char** argv)
{
  ...
  // 讀取新參數
  pnh.param("check_front", check_front, true);
  pnh.param("check_back",  check_back,  true);
  pnh.param("front_stop_distance", front_stop_distance, 0.8);
  pnh.param("back_stop_distance",  back_stop_distance,  0.8);
  ...
  // 根據開關決定是否訂閱，避免處理不需要的資料
  ros::Subscriber sub_front;
  if (check_front) {
    sub_front = nh.subscribe<sensor_msgs::LaserScan>(front_scan_topic, 10, FrontScanCallback);
  }
  // ros::Subscriber sub_back  = nh.subscribe<sensor_msgs::LaserScan>(back_scan_topic,  10, BackScanCallback);
  ros::Subscriber sub_back;
  if (check_back) {
    sub_back = nh.subscribe<sensor_msgs::LaserScan>(back_scan_topic,  10, BackScanCallback);
  }
  ...
  while (ros::ok())
  {
    ...
    // -->
    // 1) 資料有效性檢查 (Fail-safe)
    // 如果 check_front 為 false，就不檢查 front_ok，直接視為通過
    const bool front_ok = !check_front || (got_front && (now - last_front_t).toSec() <= scan_timeout);
    const bool back_ok  = !check_back  || (got_back  && (now - last_back_t ).toSec() <= scan_timeout);
    const bool cmd_ok   = got_cmd   && (now - last_cmd_t  ).toSec() <= cmd_timeout;
    ...
    // 2) 障礙判斷：分別判斷前後方
    const bool obstacle_now =
      (check_front && front_min_distance <= front_stop_distance) || (check_back  && back_min_distance  <= back_stop_distance);
