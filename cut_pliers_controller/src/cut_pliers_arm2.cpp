...
// 用於接收下位機傳回的狀態
int32_t R_H2;      // 實際手臂2高度（毫米）
int32_t R_L2;      // 實際手臂2長度（毫米）
...
// forward declaration
void send_data(void);
void receive_and_process_data(bool enable_log_);
...
void receive_and_process_data(bool enable_log_)
{
    ...
    if (n > 0)
    {
        ...
        if (buffer[0] == 0XAA)
        {
            ...
            if (buffer[0] == 0XAA && buffer[1] == 0XF1)
            {
                ...
                {
                    b++; Flag_OK = 1;
                    R_H2 = (int32_t)((buffer[11] << 0) | (buffer[12] << 8) | (buffer[13] << 16) | (buffer[14] << 24));//毫米
                    R_L2 = (int32_t)((buffer[15] << 0) | (buffer[16] << 8) | (buffer[17] << 16) | (buffer[18] << 24));//毫米
                    R_En3 = buffer[21];
                    R_En4 = buffer[22];
                    R_C2 = buffer[24];
    ...
    if (++count_1 > 149 && enable_log_) {//顯示頻率降低
        count_1 = 0;
...
public:
    CmdCutPliersPublisher(ros::NodeHandle& nh)
    {
        ...
        nh.param<std::string>("cmd_topic", cmd_topic_, std::string("/cmd_cut_pliers_2"));
        ...
        nh.param<bool>("log", enable_log_, false);
...
private:
    ...
    {
        bool updated = false;
        const int current_len = R_L2;

        if (msg->length2 >= 0)
        {
            const int req = msg->length2;
            ...
            if (msg->mode == 0)
            {
                if (req < current_len)
                {
                    log("Reject forward: req=%d < current_len=%d (mode=0)",
                        req, current_len);
                }
                else
                {
                    target_length_ = req;
                    last_valid_length_ = req;
                    updated = true;
                }
            }
            // mode 1 = 後退
            else if (msg->mode == 1)
            {
                if (!allow_retract_)
                {
                    log("Reject retract: allow_retract=false req=%d cur=%d",
                        req, current_len);
                }
                else if (req > current_len)
                {
                    log("Reject retract: req=%d > current_len=%d (mode=1)",
                        req, current_len);
                }
                else
                {
        ...
        if (updated)
        {
            log("Updated target: height2=%d length2=%d mode=%d",
                target_height_, target_length_, (int)msg->mode);
        }
    }
    ...
    void timerCallback(const ros::TimerEvent&)
    {
        ...
        if (target_length_ < 0) target_length_ = R_L2;
        S_L2 = std::min(std::max(target_length_, 10), 440);
        S_C2 = claw_;
...
int main(int argc, char** argv)
{
    ...
    // 用 private NodeHandle，讓 <param> 直接掛在 ~ 底下
    ros::NodeHandle nh("~");
    ...
    bool enable_log_;
    nh.param<std::string>("port", port, std::string("/dev/ttyUSB0"));
    ...
    // 使用 AsyncSpinner 多線程執行所有 callback
    ros::AsyncSpinner spinner(4);
    spinner.start();

    ros::Rate loop_rate(150);
    while (ros::ok()) {
        receive_and_process_data(enable_log_);
        loop_rate.sleep();
    }
